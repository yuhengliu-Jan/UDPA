...
def broadcast123(uav, node_list, type, eps, fc, Pt):
    node_num = []
    node_inti = []
    node_inf = []
    mmm = 0
...
def kenengxing2(dist):
    fc = 2
    theta = math.degrees(math.atan(0.3 / dist))
    PL = (20 * math.log10(math.sqrt(0.09 + pow(dist, 2)))) + (20 * math.log10(fc)) + 92.4
    ca = 100 - PL
    if (theta > 10):
        g = -94.2
        h = -3.44
...
def UAV_Information_collection(drone, node_list, girl_list, neib, enenti, ntemp, aall, ini):
    ...
    if (ag == 0):
        ...
        addx = 150 * math.cos(FFF)
        addy = 150 * math.sin(FFF)
        drone = [drone[0] + addx, drone[1] + addy, 300]
        print('位置', drone)
    ...
    if (ag == 1):
        ...
        stp = math.sqrt(pow(sx, 2) + pow(sy, 2)) / 150
        sx = sx / stp
        sy = sy / stp
...
def deal_all_inf(node_inf, init_position_matrix, alnodes,hisl):
    ...
    demo = althree.PSO(alnodes, init_position_matrix, zob_map, 10)
    demo.fanwei()
    demo.initial()
    demo.solving(15)
    result = demo.returnbest()
    result.sort(key=takeFirst, reverse=True)
    locm = [result[0][1], result[0][2], 300]
...
def adaptive_dep(uav_pos, zob_map, init_position_matrix, locm, alnodess, ctime, hisl):
    ...
    '''
    print("------------------------split line------------------------")
    demo = althree.PSO(alnodess, init_position_matrix, newml, ctime)
    demo.fanwei()
    demo.initial()
    demo.solving(15)
    result = demo.returnbest()
...
# movement_matrix, init_position_matrix = Gm.get_position('2020-7-2nodetype2.tcl')
# movement_matrix, init_position_matrix = Gm.get_position('2020-7-3-0.40.7type2.tcl')
...
#movement_matrix, init_position_matrix = Gm.get_position('2020-8-16-52.tcl')
#movement_matrix, init_position_matrix = Gm.get_position('2020-8-23-xiu.tcl')
