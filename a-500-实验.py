...
def kenengxing2(dist):
    fc = 2
    theta = math.degrees(math.atan(0.5 / dist))
    PL = (20 * math.log10(math.sqrt(0.25 + pow(dist, 2)))) + (20 * math.log10(fc)) + 92.4
    ca = 100 - PL
    if (theta > 10):
        g = -94.2
        h = -3.44
...
def UAV_Information_collection(drone, node_list, girl_list, neib, enenti, ntemp, aall, ini):
    ...
    if (ag == 0):
        ...
        addx = 170 * math.cos(FFF)
        addy = 170 * math.sin(FFF)
        drone = [drone[0] + addx, drone[1] + addy, 500]
        print('位置', drone)
    ...
    if (ag == 1):
        ...
        stp = math.sqrt(pow(sx, 2) + pow(sy, 2)) / 170
        sx = sx / stp
        sy = sy / stp
...
def deal_all_inf(node_inf, init_position_matrix, alnodes,hisl):
    ...
    demo = alfive.PSO(alnodes, init_position_matrix, zob_map, 10)
    demo.fanwei()
    demo.initial()
    demo.solving(15)
    result = demo.returnbest()
    result.sort(key=takeFirst, reverse=True)
    locm = [result[0][1], result[0][2], 500]
...
def adaptive_dep(uav_pos, zob_map, init_position_matrix, locm, alnodess, ctime, hisl):
    ...
    '''
    print("------------------------split line------------------------")
    demo = alfive.PSO(alnodess, init_position_matrix, newml, ctime)
    demo.fanwei()
    demo.initial()
    demo.solving(15)
    result = demo.returnbest()
