...
def kenengxing2(dist):
    fc = 2
    theta = math.degrees(math.atan(0.6 / dist))
    PL = (20 * math.log10(math.sqrt(0.36 + pow(dist, 2)))) + (20 * math.log10(fc)) + 92.4
    ca = 100 - PL
    if (theta > 10):
        g = -94.2
        h = -3.44
...
def UAV_Information_collection(drone, node_list, girl_list, neib, enenti, ntemp, aall, ini):
    ...
    if (ag == 0):
        ...
        drone = [drone[0] + addx, drone[1] + addy, 600]
        print('位置', drone)
...
def deal_all_inf(node_inf, init_position_matrix, alnodes,hisl):
    ...
    demo = alsix.PSO(alnodes, init_position_matrix, zob_map, 10)
    demo.fanwei()
    demo.initial()
    demo.solving(15)
    result = demo.returnbest()
    result.sort(key=takeFirst, reverse=True)
    locm = [result[0][1], result[0][2], 600]
    print(result[0])
...
def adaptive_dep(uav_pos, zob_map, init_position_matrix, locm, alnodess, ctime, hisl):
    ...
    '''
    print("------------------------split line------------------------")
    demo = alsix.PSO(alnodess, init_position_matrix, newml, ctime)
    demo.fanwei()
    demo.initial()
    demo.solving(15)
    result = demo.returnbest()
        ...
        for tatq in alnodes:
            ...
            for tat1 in zob_map[int(tatq)]:
                if (xxx1[int(tatq)][1] >= tat1[3] and xxx1[int(tatq)][1] <= tat1[5]):
                    if(tat1[2] == -1):
                        tiao = 1
                        tee = math.sqrt(pow(1068, 2) - pow(init_position_matrix[int(tatq)][0, 1] - xyz[0], 2) - pow(
                            init_position_matrix[int(tatq)][0, 2] - xyz[1], 2))
