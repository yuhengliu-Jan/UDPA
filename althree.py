    ...
    def objfun(self, post):
        ...
        for max in self.map:
            ...
            if (max in self.alnodes):
                ...
                tat = math.degrees(math.atan(298.5 / dis))
                # DENSE
                plos1 = (120 - (120 / (1 + pow(tat / 24.3, 1.229)))) / 100
                # URBAN
                #plos1 = (187.3 - (187.3 / (1 + pow(tat / 82.1, 1.478)))) / 100

                for suan in range(len(self.map[max]) - 1):
                    if (temij[0][1] > self.map[max][suan][3] and temij[0][1] <= self.map[max][suan][5] and dis < 1025):
